...
const fetchSearchConsoleData = async (domain: DomainType, days: number, type?: string, api?: SCAPISettings): Promise<fetchConsoleDataResponse> => {
   ...
   // Try OAuth 2.0 First (Multi-Tenant)
   try {
      // @ts-ignore
      if (domain.user_id) {
         // @ts-ignore
         const accessToken = await import('./googleOAuth').then(mod => mod.getValidGoogleToken(domain.user_id));
      ...
      console.log('[WARN] OAuth Token fetch failed:', error);
   }

   // Fallback to Service Account (Legacy / Admin)
   if (!authClient) {
...
/**
 * The function reads and returns the domain-specific data stored in the Database.
 * @param {string} domainName - The `domain` parameter is a string that represents the domain for which the SC data is being read.
 * @returns {Promise<SCDomainDataType>}
 */
export const readLocalSCData = async (domainName: string): Promise<SCDomainDataType | false> => {
   try {
      const dbDomain = await Domain.findOne({ where: { domain: domainName } });
      if (dbDomain && dbDomain.search_console_data) {
         // If stored as JSON type in DB, sequelize parses it automatically. If it's a string, verify.
         // DataType.JSON should handle it unless it was double stringified.
         return dbDomain.search_console_data;
      }
      return { threeDays: [], sevenDays: [], thirtyDays: [], lastFetched: '', lastFetchError: '' };
   } catch (error) {
      return false;
   }
};
 ...
 * @param {string} domainName - The `domain` parameter is a string that represents the domain for which the SC data will be written.
 ...
 */
export const updateLocalSCData = async (domainName: string, scDomainData?: SCDomainDataType): Promise<SCDomainDataType | false> => {
   try {
      const emptyData: SCDomainDataType = { threeDays: [], sevenDays: [], thirtyDays: [], lastFetched: '', lastFetchError: '' };
      const dataToSave = scDomainData || emptyData;

      await Domain.update({ search_console_data: dataToSave }, { where: { domain: domainName } });
...
/**
 * The function removes the domain-specific Seach Console data stored in the Database.
 * @param {string} domainName - The `domain` parameter is a string that represents the domain for which the SC data file will be removed.
 * @returns {Promise<boolean>} - Returns true if file was removed, else returns false.
 */
export const removeLocalSCData = async (domainName: string): Promise<boolean> => {
   try {
      await Domain.update({ search_console_data: null }, { where: { domain: domainName } });
      return true;
   } catch (error) {
      return false;
   }
