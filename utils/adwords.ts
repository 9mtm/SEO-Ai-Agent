 ...
 */
export const getAdwordsCredentials = async (userId?: number): Promise<false | AdwordsCredentials> => {
   try {
      const Setting = (await import('../database/models/setting')).default;
      ...
      try {
         // Global credentials
         decryptedSettings.client_id = settings.adwords_client_id ? cryptr.decrypt(settings.adwords_client_id) : (process.env.GOOGLE_CLIENT_ID || '');
         decryptedSettings.client_secret = settings.adwords_client_secret ? cryptr.decrypt(settings.adwords_client_secret) : (process.env.GOOGLE_CLIENT_SECRET || '');
         decryptedSettings.developer_token = settings.adwords_developer_token ? cryptr.decrypt(settings.adwords_developer_token) : (process.env.ADWORDS_DEVELOPER_TOKEN || '');

         if (userId) {
            const { getValidGoogleToken } = await import('./googleOAuth');
            const accessToken = await getValidGoogleToken(userId);
            if (accessToken) {
               decryptedSettings.access_token = accessToken;
               // Dummy refresh token to enable flow, since we handled auth already
               decryptedSettings.refresh_token = 'oauth_handled';
            }
            // Try to get account_id from user settings or similar if we had it.
            // For now, we rely on the caller or default.
            decryptedSettings.account_id = settings.adwords_account_id ? cryptr.decrypt(settings.adwords_account_id) : '';
         } else {
            // Legacy
            decryptedSettings.account_id = settings.adwords_account_id ? cryptr.decrypt(settings.adwords_account_id) : '';
            decryptedSettings.refresh_token = settings.adwords_refresh_token ? cryptr.decrypt(settings.adwords_refresh_token) : '';
         }
      ...
      return decryptedSettings as AdwordsCredentials;
   } catch (error) {
 ...
 */
export const getAdwordsAccessToken = async (credentials: AdwordsCredentials | any) => {
   const { client_id, client_secret, refresh_token, access_token } = credentials;
...
export const getAdwordsKeywordIdeas = async (credentials: AdwordsCredentials, adwordsDomainOptions: IdeaSettings, test: boolean = false) => {
   ...
   let { account_id, developer_token } = credentials;
   const { country = '2840', language = '1000', keywords = [], domain = '', seedType } = adwordsDomainOptions || {};

   console.log('[DEBUG] Starting getAdwordsKeywordIdeas');
   console.log(`[DEBUG] Creds present. Account ID: ${account_id ? 'YES' : 'NO'}, Dev Token: ${developer_token ? 'YES' : 'NO'}`);

   let accessToken = '';

   // const cachedAccessToken:string|false|undefined = memoryCache.get(`adwords_token_${account_id}`);
   // if (cachedAccessToken && !test) {
   //    accessToken = cachedAccessToken;
   // } else {
   accessToken = await getAdwordsAccessToken(credentials);
   // We don't cache here anymore to avoid stale tokens, we rely on the DB/Auth system
   ...
   if (accessToken) {
      ...
      try {
         const tokenInfoResp = await fetch(`https://www.googleapis.com/oauth2/v1/tokeninfo?access_token=${accessToken}`);
         const tokenInfo = await tokenInfoResp.json();
         console.log('[DEBUG] Token Info - Scopes:', tokenInfo.scope);
         console.log('[DEBUG] Token Info - ClientID:', tokenInfo.issued_to);
      } catch (e) {
         console.log('[DEBUG] Check token info failed:', e);
      }

      // Auto-discover Account ID if missing
      developer_token = developer_token ? developer_token.trim() : '';

      if (!account_id) {
         console.log('[DEBUG] Account ID missing. Attempting to list accessible customers...');
         try {
            const custResp = await fetch('https://googleads.googleapis.com/v18/customers:listAccessibleCustomers', {
               method: 'GET',
               headers: {
            ...
            if (custData.resourceNames && custData.resourceNames.length > 0) {
               ...
               const firstCustomer = custData.resourceNames[0];
               account_id = firstCustomer.split('/')[1];
               console.log(`[DEBUG] Found accessible customer: ${account_id}`);
            } else {
               console.log('[ERROR] No accessible customers found for this Google Account.');
               console.log('[DEBUG] Customer List Response:', JSON.stringify(custData));
            }
         } catch (e) {
            console.log('[ERROR] Failed to list accessible customers:', e);
         }
      }

      if (!account_id) {
         console.log('[ERROR] Cannot proceed without Account ID.');
         return [];
      }
      ...
      try {
         ...
         console.log(`[DEBUG] Using Customer ID: ${customerID}`);
         ...
         const resp = await fetch(`https://googleads.googleapis.com/v17/customers/${customerID}:generateKeywordIdeas`, {
            method: 'POST',
            headers: {
               'Content-Type': 'application/json',
               'developer-token': developer_token,
         ...
         if (ideaData?.results) {
            ...
            console.log(`[DEBUG] Fetched ${fetchedKeywords.length} keywords.`);
         }
