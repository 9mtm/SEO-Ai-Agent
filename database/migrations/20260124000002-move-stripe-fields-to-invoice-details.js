...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const invoiceColumns = await queryInterface.describeTable('invoice_details');

        if (!invoiceColumns.stripe_customer_id) {
            await queryInterface.addColumn('invoice_details', 'stripe_customer_id', {
                type: Sequelize.STRING,
                allowNull: true
            });
        }
        if (!invoiceColumns.stripe_subscription_id) {
            await queryInterface.addColumn('invoice_details', 'stripe_subscription_id', {
                type: Sequelize.STRING,
                allowNull: true
            });
        }
        if (!invoiceColumns.stripe_current_period_end) {
        ...
        if (userColumns.stripe_subscription_id) {
            await queryInterface.removeColumn('users', 'stripe_subscription_id');
        }
        if (userColumns.stripe_current_period_end) {
