...
type TopbarProps = {
   ...
   showSettings?: () => void, // Add optional prop usually passed by Research page if needed, or check usage
}

const TopBar = ({ domains = [], currentDomain }: TopbarProps) => {
   const router = useRouter();


   return (
      <div className={`topbar flex w-full mx-auto justify-between 
       max-w-7xl lg:justify-end  bg-white lg:bg-transparent`}>
         ...
         {router.asPath !== '/research' && (
            <Link href={'/'} className=' right-14 top-2 px-2 py-1 cursor-pointer bg-[#ecf2ff] hover:bg-indigo-100 transition-all
